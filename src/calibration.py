"""
Development-only calibration assessment for the selected logistic model.

The calibration method was pre-specified as sigmoid / Platt scaling.

This script produces out-of-fold predictions so calibration quality is not
assessed on observations used to fit the corresponding estimator.
...
SUMMARY_PATH = (
    OUTPUT_DIR
    / "calibration_development_summary.json"
)

RELIABILITY_PATH = (
    FIGURES_DIR
    / "development_reliability_diagram.png"
)


def make_base_pipeline(X_train):
    """
...
def main():
    ...
    # ---------------------------------------------------------
    # Outer folds are used only to create honest OOF predictions.
    # ---------------------------------------------------------
    ...
    print()
    print("=" * 72)
    print("DEVELOPMENT CALIBRATION ASSESSMENT")
    print("=" * 72)
    ...
    for fold_number, (
        ...
        #
        # Calibration CV occurs entirely inside the outer
        # training fold.
        # -----------------------------------------------------
        ...
        print(
            ...
            f"calibrated={calibrated_brier:.5f}"
        )
    ...
    plt.title(
        "Development Reliability Diagram"
    )
    ...
    print(
        f"Selected raw @0.40 : "
        f"{raw_selected:,} "
        f"({raw_selected_pct:.2%})"
    )

    print(
        f"Selected cal @0.40 : "
        f"{calibrated_selected:,} "
        f"({calibrated_selected_pct:.2%})"
    )
    ...
    print()
    print(
        "SIGMOID WAS PRE-SPECIFIED. "
        "DO NOT SWITCH CALIBRATION METHOD "
