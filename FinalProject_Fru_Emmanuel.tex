...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{/mnt/data/final_report/assets/architecture.png}
...
\toprule
Model & PR-AUC & ROC-AUC & F1 & Accuracy & Brier \\
\midrule
Majority baseline & 0.2654 & 0.5000 & 0.0000 & 0.7346 & 0.1949 \\
Logistic regression & \textbf{0.6549} & \textbf{0.8437} & \textbf{0.5967} & \textbf{0.8041} & \textbf{0.1361} \\
HistGradientBoosting & 0.6425 & 0.8339 & 0.5746 & 0.7949 & 0.1413 \\
ANN & 0.6532 & 0.8414 & 0.5866 & 0.8014 & 0.1370 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering\scriptsize
\caption{Final holdout fairness audit}
\begin{tabular}{llrrrrrr}
\toprule
Attribute & Group & $n$ & Churn & Selected & Precision & Recall & FPR \\
\midrule
Gender & Female & 722 & 26.18\% & 30.06\% & 58.99\% & 67.72\% & 16.70\% \\
Gender & Male & 687 & 26.93\% & 28.68\% & 59.90\% & 63.78\% & 15.74\% \\
SeniorCitizen & 0 & 1,168 & 22.77\% & 25.60\% & 55.18\% & 62.03\% & 14.86\% \\
SeniorCitizen & 1 & 241 & 44.81\% & 47.72\% & 70.43\% & 75.00\% & 25.56\% \\
\bottomrule
\end{tabular}
\end{table}
...
Generative AI was used as a development assistant, not as an autonomous evaluator. Claude was used to help create the initial project brief and for a final review of the completed work. ChatGPT was used more extensively for methodological review, Python development and debugging, interpretation of experimental outputs, architecture design, fairness and business-impact analysis, and drafting/refining the report and supporting documentation. Model training, testing, and the final holdout evaluation were executed locally.

\subsection{Tools and specific applications}

\begin{table}[H]
\centering\scriptsize
\caption{AI tools and their project roles}
\begin{tabularx}{\textwidth}{p{0.16\textwidth}p{0.34\textwidth}Y}
\toprule
Tool & Specific use & Verification \\
\midrule
Claude & Initial project brief; final review of completed work & Compared against assignment requirements, project artifacts, and the final evidence set \\
ChatGPT & Protocol review; Python code drafting/debugging; interpretation of CV, calibration, ablation, fairness, holdout and business outputs & Local execution, persisted CSV/JSON outputs, automated tests, manual recalculation \\
ChatGPT & Azure architecture and interoperability planning; report, Model Card, README drafting & Primary-source verification and manual editing \\
ChatGPT image generation & Architecture visual from an approved technical specification & First version rejected; second academic-style diagram manually reviewed and accepted \\
\bottomrule
\end{tabularx}
\end{table}
\takeaway{AI accelerated planning, implementation, review, and drafting, but empirical results and final methodological decisions remained tied to locally generated artifacts and pre-registered rules.}

\subsection{Refinement episodes}

\textbf{Episode 1: evaluation protocol.} The exact project-review prompt was: \emph{``I've attached my final project and a brief I'd like to stick to in drafting this. Review both documents and flag any anomaly before we start drafting the final project.''} The review identified that the Module 6 test set had already been inspected. The protocol was revised before experimentation to create a new persisted 80/20 split and reserve the 1,409-row holdout until freeze. The limitation that this is still the same underlying source dataset was retained explicitly.

\textbf{Episode 2: model complexity and calibration.} Development outputs were supplied for interpretation rather than replaced with AI estimates. Logistic regression, HGB, and ANN were practically tied under the locked paired-fold rule, so logistic regression was selected for simplicity. Later, sigmoid calibration slightly worsened Brier score on both development and holdout data. Rather than search post hoc for another calibrator, the pre-specified method was retained and the failed hypothesis was reported.

\textbf{Episode 3: post-holdout reporting bug.} After the one-time holdout run, AI review noticed that the classification summary selected 414 customers at 0.40 while the printed business summary selected 415. The discrepancy came from using the exact 0.398860 threshold inside the sensitivity function. The model was not retrained and the holdout was not rerun. A separate post-processing script recomputed the operational business result from immutable saved predictions, producing 414 selected customers and 47.28\% expected ROI.

\textbf{Episode 4: architecture visual.} The first generated architecture graphic contained the right components but was visually dense. The exact refinement prompt was: \emph{``architecture diagram looks too noisy. I need a standard scientific paper type diagram that is academically acceptable. Not AI slop.''} The first visual was rejected and replaced by the simpler technical diagram used in Section 4.

\subsection{Verification and reflection}

Verification methods included local Python execution, persistent CSV/JSON artifacts, manual arithmetic checks, primary-source reference verification, frozen configuration artifacts, and an automated test suite. All 11 tests passed before the holdout was opened.

The most useful AI interactions were often those that produced a correction rather than a finished answer. The evaluation protocol changed, the expected model ordering was not supported, the calibration hypothesis failed, removing demographic variables did not eliminate group disparities, a post-holdout reporting inconsistency was corrected without rerunning the model, and the first architecture visual was rejected. These cases reinforced that AI output should be treated as a draft, hypothesis, or review input rather than evidence. Responsibility for accepting methodological changes, interpreting results, and approving the final submission remained with the student.

\takeaway{Responsible AI use in this project is demonstrated by traceable prompts, rejected outputs, failed hypotheses, independent verification, and preservation of the locked evaluation protocol.}
...
\newpage
\appendix
\section{Selected Verbatim AI Prompt Evidence}

The detailed project AI log should be submitted with the supporting artifacts. The following prompts are reproduced verbatim because they are available directly from the project conversation history.

\subsection*{Project brief review}
\begin{quote}\small
...
\begin{quote}\small
architecture diagram looks too noisy. I need a standard scientific paper type diagram that is academically acceptable. Not AI slop.
\end{quote}

Where an interaction consisted primarily of terminal output, screenshots, or short continuation messages rather than a substantive written prompt, the AI log describes the interaction honestly instead of reconstructing a polished prompt after the fact. Claude is disclosed for the initial brief and final review. Exact Claude prompt text is not reproduced here because it was not retained in this conversation record; no prompt was reconstructed after the fact.
